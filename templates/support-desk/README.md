...
A standalone MOVP support desk using tickets-as-tasks, SLA automation, and the platform inbox.

| Layer | Collections |
| --- | --- |
| Platform | `task`, `task_status_option`, `task_priority_option`, `automation_rule`, `comment`, `event_type` |
| Project | `support_ticket`, `sla_policy` |

Each support ticket links to a platform task for status, priority, assignment, and due-date behavior.
...
The local Supabase API is isolated on port `64721`. Set `SUPABASE_ANON_KEY` in
