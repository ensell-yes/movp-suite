...
A standalone MOVP knowledge base with embeddable product documentation and hybrid search.

| Layer | Collections |
| --- | --- |
| Platform | `content_item`, `saved_item`, `tag`, search and embedding infrastructure |
| Project | `kb_article`, `kb_category` |

Hybrid search indexes `kb_article.body` because the project schema marks it as embeddable.
...
The local Supabase API is isolated on port `64821`. Set `SUPABASE_ANON_KEY` in
