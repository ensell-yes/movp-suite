...
> | C3 Agent Connectivity (PATs/MCP/CLI) | `2026-07-09-movp-stage-c-03{a,b,c,d}-*.md` (+ design spec) | ✅ EXECUTED (C3a PAT foundation; C3b CLI parity; C3c MCP matrix + `@movp/mcp-bridge` + docs; C3d `[agents]` slice; local full gate + PR #11 CI green incl. `slice-e2e: PASS`). Follow-up: post-release MCP-doc credential hardening — placeholder-only sample auth (no literal `movp_pat_`; Codex native streamable HTTP, retired `experimental_use_rmcp_client`), PAT-as-user-scoped guidance, and a regression-tested `check-agent-configs` lint (`scripts/lib/agent-config-checks.mjs`); merged PR #29 `7a70356`, CI green. |
...
> | C7 Inline Editing & Delivery | breakdown + `2026-07-15-c7.1-editor-dependency-spike.md` + `2026-07-16-movp-stage-c-07{b,c,d}-*.md` + approved tail design + `2026-07-23-movp-stage-c-07{e,f,g,h,i}-*.md` | 🟡 C7.1–C7.3 EXECUTED; C7.3a (richtext canonicalization in `@movp/richtext` + hash-first idempotent `update_content`) and C7.3b (editor-SDK dirty/non-destructive conflict recovery, sanitized GraphQL `CONFLICT` boundary, bounded/observable richtext endpoint, shared-revision `RichTextFieldsIsland`, CMS page binding) COMPLETE — PR #26 CI green on `453f5b3`, incl. `slice-e2e: PASS` (clears the earlier `gte-small` download stop), publishable-versions, docs-freshness, workspace typecheck, `c7-editor-sdk`, and all four template-smoke (SDK 32, GraphQL 61, frontend unit/component 41, frontend e2e 86); reviewed 9.26. C7.4–C7.7 tail design reviewed 9.25. 07e delivery core is EXECUTED: forward-only published-read RPCs, reserved/unique typed routes, allowlisted `@movp/delivery` renderer, bounded artifacts, fail-loud schema-driven anonymous Astro delivery routes, canonical exact-once public-read/artifact observability pulled forward from 07h, and the required package/release/`c7-delivery` gates are landed. 07f privileged overlay is EXECUTED: owner/admin edit-capability RLS and caller-bound asset guards, the canonical hash-first `updateRichTextField` mutation/proxy path, the client-safe overlay package, strict-CSP interaction-deferred capability bootstrap, anonymous bundle exclusion, accessibility/conflict gates, and required `c7-inline-overlay` CI wiring are landed. 07e+07f MERGED to `main` via PR #33 (`a441397`, 2026-07-25) with all 32 CI checks green; the merge additionally fixed a workerd request-body drain regression (responding with an unconsumed body corrupts the next request on the connection), the `c7-inline-overlay` job ordering (`supabase start` now precedes the DB-backed domain suite), and the `create-movp` frontend binding inventory (`PUBLIC_SITE_URL`). Execution continues with 07g private Realtime → 07h SEO/cache-withdrawal operability → 07i lifecycle slice; 07g–07i remain PLANNED, UNEXECUTED, so C7 as a phase is NOT done. |
