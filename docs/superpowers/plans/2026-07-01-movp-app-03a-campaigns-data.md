...
- **Contract fidelity (Parts B/C depend on these):**
  ...
  - **Two event payloads:** `campaign.created` = `{id, entity_type:'campaign', entity_id, status, marketing_plan_id}`; `deliverable.created` = `{id, entity_type:'campaign_deliverable', entity_id, deliverable_type, campaign_id}` — ids/classifiers only, audit-only, no `notify` job. `entity_type`/`entity_id` are the cross-part contract (Part C's e2e + the inbox key on `entity_id`) — pinned by count + classifier reads + two `entity_id`-equals-row-id reads + SQL assertions that no notify jobs are created for these audit-only events.
